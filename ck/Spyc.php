...
class Spyc {
	...
	function _replace(& $item, $key, $replace) {
		$item = str_replace ( $replace ['search'], $replace ['replace'], $item );
	}
	
	function load($input, $replace = null) {
		$Source = $this->loadFromSource ( $input );
		if (empty ( $Source ))
			return array ();
		
		if (is_array ( $replace )) {
			$replace = array ('search' => array_keys ( $replace ), 'replace' => array_values ( $replace ) );
		...
		for($i = 0; $i < count ( $Source ); $i ++) {
			...
			if ( ($literalBlockStyle = $this->startsLiteralBlock ( $line )) != null ) {
	...
	function _parseLine($line) {
		...
		if ( ($group = $this->nodeContainsGroup ( $line )) != null ) {
